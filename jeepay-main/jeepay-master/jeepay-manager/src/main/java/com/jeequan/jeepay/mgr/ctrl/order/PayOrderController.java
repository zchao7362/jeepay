...
public class PayOrderController extends CommonCtrl {
     ...
     * @date 2021/6/17 16:38
     */
    @MethodLog(remark = "发起订单回调")
    @PreAuthorize("hasAuthority('ENT_PAY_ORDER_REFUND')")
    @PostMapping("/callbackOrder/{payOrderId}")
    public ApiRes callbackOrderUp(@PathVariable("payOrderId") String payOrderId){
        PayOrder payOrder = payOrderService.getById(payOrderId);
        if (payOrderService.callbackOrder(payOrder)){
            return ApiRes.ok();
        }else {
            return ApiRes.fail(ApiCodeEnum.CALLBACK_ERROR);
        }
    }
}