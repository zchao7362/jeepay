...
public abstract class ApiController extends AbstractCtrl {
    ...
    /** 获取请求参数并转换为对象，商户通用验证  **/
    protected <T extends AbstractRQ> T getRQByWithCallbackSign(Class<T> cls){

        //获取请求RQ, and 通用验证
        T bizRQ = getRQ(cls);

        CallbackOrderRQ callbackOrderRQ = (CallbackOrderRQ)bizRQ;

        String sign = bizRQ.getSign();

        String appSecret = SecureUtil.sha256(callbackOrderRQ.getMchNo());
