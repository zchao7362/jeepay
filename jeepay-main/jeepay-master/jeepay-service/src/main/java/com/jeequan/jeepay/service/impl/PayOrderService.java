...
public class PayOrderService extends ServiceImpl<PayOrderMapper, PayOrder> {
    ...
    /** 查询商户订单 **/
    public PayOrder queryZftOrder(String mchNo, String payOrderId, String mchOrderNo){

        if(StringUtils.isNotEmpty(payOrderId)){
            return getOne(PayOrder.gw().eq(PayOrder::getZftMchNo, mchNo).eq(PayOrder::getPayOrderId, payOrderId));
        }else if(StringUtils.isNotEmpty(mchOrderNo)){
            return getOne(PayOrder.gw().eq(PayOrder::getZftMchNo, mchNo).eq(PayOrder::getMchOrderNo, mchOrderNo));
        }else{
            return null;
        }
    }
    ...
    public JSONObject mainPageWeekCount(String mchNo) {
        ...
        for(int i = 0 ; i < 7 ; i++){
         ...
         dayStart = DateUtil.beginOfDay(today).toString(DatePattern.NORM_DATETIME_MINUTE_PATTERN);
         dayEnd = DateUtil.endOfDay(today).toString(DatePattern.NORM_DATETIME_MINUTE_PATTERN);
        Map param = new HashMap<>();
        param.put("state", TransferOrder.STATE_SUCCESS);
        param.put("createTimeStart", dayStart);
        param.put("mchNo", mchNo);
        param.put("createTimeEnd", dayEnd);
    ...
    */
    ...
    public boolean callbackOrder(PayOrder payOrder){
        MchApp mchApp = mchAppService.getOneByMch(payOrder.getMchNo(), payOrder.getAppId());
        String appSecret = mchApp.getAppSecret();
        //如果订单支付成功，和回调未成功，调用异步回调地址
        if(payOrder.getState() == 2 || payOrder.getNotifyState() != 1){
            //异步回调地址
            String callbackUrl = payOrder.getNotifyUrl();
            Map<String, Object> params = new HashMap<String, Object>();
            params.put("mchNo", payOrder.getMchNo());
            params.put("appId", payOrder.getAppId());
            params.put("mchOrderNo", payOrder.getMchOrderNo());
            params.put("orderState", payOrder.getState());
            params.put("callbackTime", System.currentTimeMillis());
            JSONObject bizReqJSON = (JSONObject)JSONObject.toJSON(params);
            String sign = JeepayKit.getSign(bizReqJSON, appSecret);
            log.info("回调订sign："+sign);
            params.put("sign", sign);
            String body = JSONUtil.toJsonStr(params);
            String res = HttpRequest.post(callbackUrl).body(body).execute().body();
            log.info("回调订单号："+payOrder.getMchOrderNo()+"，回调反馈值："+res);
            if("success".equals(res)){
                payOrder.setNotifyState((byte) 1);
                payOrder.setNotifyTime(new Date());
                if(saveOrUpdate(payOrder)){
                    log.info("回调订单号："+payOrder.getMchOrderNo()+"，成功");
                }
                return true;
            }else {
                log.info("回调订单号："+payOrder.getMchOrderNo()+"，失败");
                return false;
            }
        }
        return true;
