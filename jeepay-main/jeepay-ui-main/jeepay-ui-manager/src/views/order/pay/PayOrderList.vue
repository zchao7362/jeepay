      ...
      >
        ...
        <template slot="opSlot" slot-scope="{record}">  <!-- 操作列插槽 -->
          <JeepayTableColumns>
            ...
            <a-button type="link" v-if="$access('ENT_PAY_ORDER_REFUND')" style="color: red" v-show="(record.state === 2 && record.notifyState !== 1)" @click="callbackOrderFunc(record.payOrderId)">回调</a-button>
          </JeepayTableColumns>
        </template>
      </JeepayTable>
    </a-card>
...
import { API_URL_PAY_ORDER_LIST, API_URL_PAYWAYS_LIST, req, payOrderNotify } from '@/api/manage'
...
export default {
  ...
  methods: {
    ...
    callbackOrderFunc: function (recordId) {
      const that = this
      payOrderNotify(recordId).then(res => {
        that.detailData = res
      })
    },
    moment,
