  ...
  <page-header-wrapper>
    <a-card>
      <div class="table-page-search-wrapper">
        <a-form layout="inline" class="table-head-ground">
          <div class="table-layer">
            ...
            <jeepay-text-up :placeholder="'手机号'" :msg="searchData.phone" v-model="searchData.phone"/>
      ...
      >
        ...
        <template slot="divisionMode" slot-scope="{record}">
          <div v-if="record.divisionMode == 0" ><a-badge status="error" text="不分账" /></div>
          <div v-else-if="record.divisionMode == 1" ><a-badge status="processing" text="自动分账" /></div>
          <div v-else ><a-badge status="warning" text="手动分账" /></div>
        </template>
        <template slot="phoneSlot" slot-scope="{record}">
          <b>{{ record.phone }}</b>
        </template> <!-- 自定义插槽 -->
...
const tableColumns = [
  ...
  { key: 'divisionMode', title: '是否分账', scopedSlots: { customRender: 'divisionMode' } },
