      ...
      <a-form>
        <div style="display:flex;flex-direction:row">
          <a-form-item label="" class="table-head-layout">
            <label>商 户 编 号：</label><span id="mchNo">{{ reqData.mchNo }}</span>
        ...
        <div style="width: 100%;" class="paydemo">
          ...
          <div class="paydemo-type-content">
            ...
            <form class="layui-form">
              ...
              <div class="paydemo-form-item">
                <label>商 户 余 额：</label><span id="availableAmount">{{ reqData.availableAmount }}</span>
                <span @click="getAvailableAmount" class=" paydemo-btn" style="padding:0 3px">查询商户余额</span>
              </div>
              <div class="paydemo-form-item">
                <span>转账金额(元)：</span>
              ...
              <div class="paydemo-form-item">
                <span>收  款  账  号：</span><a-input v-model="reqData.accountNo" style="width: 300px; margin-right: 10px"/>
              ...
              <div class="paydemo-form-item" style="margin-top: 10px">
                <span>手 机 号 码：</span><a-input v-model="reqData.phone" style="width: 200px"/>
              </div>
              <div class="paydemo-form-item" style="margin-top: 10px">
                <span>验   证   码：</span><a-input v-model="reqData.phoneCode" style="width: 200px"/>
                <span v-show="show"  @click="sendCode" class="paydemo-btn" style="padding:0 3px">获取验证码</span>
                <span v-show="!show" class="paydemo-btn">{{count}}s后重新获取</span>
              </div>
              <div class="paydemo-form-item">
...
export default {
  ...
  mounted () {
    ...
    req.list(API_URL_MCH_APP, { pageSize: -1 }).then(res => {
      ...
      if (that.mchAppList.length > 0) {
        ...
        that.reqData.mchNo = that.mchAppList[0].mchNo
        this.getMchInfo()
        // 根据不同的appId展示不同的支付方式
  ...
  methods: {
    ...
    getAvailableAmount () {
      const that = this
      queryBalance(that.reqData).then(res => {
        that.reqData.availableAmount = res.availableAmount
      })
    },
    sendCode () {
      const that = this
      this.reqData.extParam = this.reqData.phone
      getCodeApi(this.reqData).then(res => {
        if (res === 1) {
          this.show = false
          this.getCode()
          const succModal = that.$infoBox.modalSuccess('验证码发送成功', <div>2s后自动关闭...</div>)
